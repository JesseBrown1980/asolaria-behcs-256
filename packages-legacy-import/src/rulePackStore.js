...
const DEFAULT_OUTPUT_PATH = path.join(projectRoot, "data", "agent-index", "rule", "PACKS.md");
const DEFAULT_POINTER_OUTPUT_PATH = path.join(projectRoot, "data", "agent-index", "rule", "POINTERS.md");

const RULE_PACK_DEFS = Object.freeze([
  Object.freeze({
    id: "identity-sovereignty",
    title: "Identity / Sovereignty",
    summary: "Local identity, sovereignty boundaries, host authority, and colony placement rules.",
    keywords: ["identity", "sovereign", "local", "rayssa", "jesse", "liris", "capital", "host", "machine"],
    preferredRoles: ["helm", "sentinel", "watchdog"]
  }),
  Object.freeze({
    id: "transport-federation",
    title: "Transport / Federation",
    summary: "Bridge, federation, transport boundaries, remote readiness, and lane rules.",
    keywords: ["bridge", "federation", "transport", "mqtt", "remote", "lane", "shared", "compute", "readiness", "handoff"],
    preferredRoles: ["omnispindle-bridge", "omnispindle-control", "omnispindle-scout", "rook"]
  }),
  Object.freeze({
    id: "authority-control",
    title: "Authority / Control",
    summary: "Lease activation, controller authority, command boundaries, and orchestration-control rules.",
    keywords: ["authority", "control", "lease", "controller", "activation", "preferred", "write-capable", "orchestration", "dispatch"],
    preferredRoles: ["helm", "sentinel", "watchdog"]
  }),
  Object.freeze({
    id: "truth-index",
    title: "Truth / Index",
    summary: "Truth precedence, indexing, destination rules, and orientation-layer rules.",
    keywords: ["truth", "index", "catalog", "running", "staging", "prod", "precedence", "orientation", "pointer", "despawn"],
    preferredRoles: ["helm", "vector", "watchdog", "forge"]
  }),
  Object.freeze({
    id: "security-safety",
    title: "Security / Safety",
    summary: "Security boundaries, stealth-port doctrine, ssh posture, and irreversible safety rules.",
    keywords: ["security", "stealth", "ssh", "plaintext", "encryption", "safety", "port", "disabled", "never"],
    preferredRoles: ["sentinel", "omnispindle-control", "watchdog"]
  }),
  Object.freeze({
    id: "execution-boundary",
    title: "Execution / Surface",
    summary: "Execution boundaries for browser actions, phone lanes, and other surface-specific rules.",
    keywords: ["browser", "whatsapp", "surface", "phone", "execute", "compose", "send", "tool", "runtime"],
    preferredRoles: ["falcon", "rook", "forge"]
  })
]);
...
function buildRuleCorpus(document) {
  return [
    document.id,
    document.title,
    document.summary,
...
function ruleDocsFromPayload(options = {}) {
  ...
  return {
    ...
    documents: (Array.isArray(payload.documents) ? payload.documents : []).filter((document) => document.type === "rule")
  };
}

function scorePack(doc, pack) {
  const corpus = buildRuleCorpus(doc);
  ...
  for (const keyword of pack.keywords) {
    ...
    if (!probe) continue;
    if (tags.includes(probe)) score += 5;
    else if (tags.some((tag) => tag.includes(probe) || probe.includes(tag))) score += 3;
    if (String(doc.title || "").toLowerCase().includes(probe)) score += 3;
    if (corpus.includes(probe)) score += 1;
  }
  return score;
}
...
function assignRulePacks(documents = []) {
  const packMap = new Map(RULE_PACK_DEFS.map((pack) => [pack.id, {
    id: pack.id,
    title: pack.title,
  ...
  packMap.set("general", {
    id: "general",
    title: "General / Misc",
    summary: "Rules that do not strongly match a narrower pack.",
    preferredRoles: [],
    keywords: [],
    count: 0,
    documents: []
  ...
  for (const document of documents) {
    let bestId = "general";
    let bestScore = 0;
    for (const pack of RULE_PACK_DEFS) {
...
function buildRulePackReport(options = {}) {
  const payload = ruleDocsFromPayload(options);
  const packs = assignRulePacks(payload.documents);
  return {
    ok: true,
    profile: payload.profile,
    totalRules: payload.documents.length,
...
function scoreRoleForPack(pack, role, config) {
  ...
  let score = 0;
  if ((pack.preferredRoles || []).includes(role)) score += 20;
  for (const term of terms) {
    if (!term) continue;
    if ((pack.keywords || []).includes(term)) score += 6;
    if ((pack.topTags || []).includes(term)) score += 4;
    if (String(pack.title || "").toLowerCase().includes(term)) score += 2;
    if (String(pack.summary || "").toLowerCase().includes(term)) score += 1;
  }
  return score;
}
...
function scoreRoleForDocument(document, role, config) {
  ...
  for (const term of terms) {
    ...
    else if (tags.some((tag) => tag.includes(term) || term.includes(tag))) score += 4;
...
function getRulePacksForRole(role, config, options = {}) {
  const report = options.report && options.report.packs ? options.report : buildRulePackReport(options);
  const scored = report.packs
    .map((pack) => ({ pack, score: scoreRoleForPack(pack, role, config) }))
  ...
  return {
    ok: true,
    role,
    totalRulePacks: report.packCount,
    packs: selected
  };
}

function buildRulePointersForRole(role, config, options = {}) {
  const report = options.report && options.report.packs ? options.report : buildRulePackReport(options);
  const selected = Array.isArray(options.visiblePacks) && options.visiblePacks.length > 0
    ? options.visiblePacks
    : getRulePacksForRole(role, config, { limit: options.limit || 2, report }).packs;
  ...
  const primaryPack = focusPacks[0]
    ? { id: String(focusPacks[0].id || "").trim(), title: String(focusPacks[0].title || "").trim(), count: Number(focusPacks[0].count || 0) }
    : null;
  ...
  for (const pack of focusPacks) {
    ...
    const scored = documents
      .map((document) => ({ document, score: scoreRoleForDocument(document, role, config) }))
  ...
  const anchors = anchorRows.filter((row) => {
    if (!row.id || seenAnchors.has(row.id)) return false;
  ...
  const matchingTags = unique(anchors.flatMap((anchor) => (Array.isArray(anchor.tags) ? anchor.tags : []).map((tag) => String(tag || "").toLowerCase()))).slice(0, 6);
  let guidance = "Use focused rule anchors before widening to raw rule search.";
  if (primaryPack) {
  ...
  return {
    ok: true,
    role,
    totalRules: Number(report.totalRules || 0),
...
function renderRulePackCatalog(report = buildRulePackReport()) {
  const lines = [
    "# Rule Packs",
    "",
    "> Auto-generated. Do NOT hand-edit.",
    `> Profile: ${report.profile} | Rules: ${report.totalRules} | Packs: ${report.packCount}`,
    "",
    "## Packs",
    ""
  ];
...
function renderRulePointerCatalog(report) {
  const pointerReport = report && Array.isArray(report.roles)
    ? report
    : buildRulePointerReport(report || {});
  const lines = [
    "# Rule Pointers",
    "",
    "> Auto-generated. Do NOT hand-edit.",
    `> Profile: ${pointerReport.profile} | Rules: ${pointerReport.totalRules} | Packs: ${pointerReport.packCount} | Roles: ${pointerReport.roleCount}`,
    "",
    "## Roles",
    ""
  ];
...
function writeRulePackCatalog(report = buildRulePackReport(), outputPath = DEFAULT_OUTPUT_PATH) {
  const content = renderRulePackCatalog(report);
...
function writeRulePointerCatalog(report, outputPath = DEFAULT_POINTER_OUTPUT_PATH) {
  const content = renderRulePointerCatalog(report);
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, content, "utf8");
  return {
    ok: true,
...
module.exports = {
  ...
  RULE_PACK_DEFS,
  buildRulePackReport,
  buildRulePointerReport,
  buildRulePointersForRole,
  getRulePacksForRole,
  renderRulePackCatalog,
  renderRulePointerCatalog,
  writeRulePackCatalog,
  writeRulePointerCatalog
};
