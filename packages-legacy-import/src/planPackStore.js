...
const DEFAULT_OUTPUT_PATH = path.join(projectRoot, "data", "agent-index", "plan", "PACKS.md");
const DEFAULT_POINTER_OUTPUT_PATH = path.join(projectRoot, "data", "agent-index", "plan", "POINTERS.md");

const PLAN_PACK_DEFS = Object.freeze([
  Object.freeze({
    id: "orchestration-roadmap",
    title: "Orchestration / Roadmap",
    summary: "Next-phase plans, startup order, controller structure, and lane sequencing.",
    keywords: ["plan", "next", "phase", "startup", "controller", "orchestration", "helm", "lane", "boot", "coordination"],
    preferredRoles: ["helm", "vector", "watchdog"]
  }),
  Object.freeze({
    id: "federation-comms",
    title: "Federation / Comms",
    summary: "Bridge, MQTT, direct communication, remote-node coordination, and colony sync plans.",
    keywords: ["bridge", "mqtt", "communication", "comms", "gaia", "direct", "federation", "remote", "node", "sync"],
    preferredRoles: ["omnispindle-bridge", "omnispindle-control", "omnispindle-scout", "rook"]
  }),
  Object.freeze({
    id: "recovery-repair",
    title: "Recovery / Repair",
    summary: "Revival, cleanup, restart, proof bundles, and protect-the-index recovery plans.",
    keywords: ["revival", "recovery", "cleanup", "restart", "protect", "index", "proof", "repair", "rollback", "ignite"],
    preferredRoles: ["helm", "rook", "watchdog"]
  }),
  Object.freeze({
    id: "external-project-delivery",
    title: "External Project / Delivery",
    summary: "QDD, eBacMap, pnpm, Mongo, and external project execution or handoff plans.",
    keywords: ["qdd", "ebacmap", "pnpm", "mongo", "ez", "protect", "branch", "delivery", "project", "scheduler"],
    preferredRoles: ["forge", "rook", "vector"]
  }),
  Object.freeze({
    id: "architecture-language",
    title: "Architecture / Language",
    summary: "ASO, unified index language, vocabulary, memory, and system-structure plans.",
    keywords: ["aso", "language", "index", "vocabulary", "memory", "architecture", "unified", "semantic", "tables", "design"],
    preferredRoles: ["vector", "helm", "forge"]
  }),
  Object.freeze({
    id: "voice-surface-product",
    title: "Voice / Surface / Product",
    summary: "Voice pipeline, device surfaces, product demos, and user-facing execution plans.",
    keywords: ["voice", "surface", "product", "demo", "phone", "pipeline", "mobile", "injection", "ui", "desktop"],
...
function buildPlanCorpus(document) {
  return [
    document.id,
    document.title,
    document.summary,
...
function planDocsFromPayload(options = {}) {
  ...
  return {
    ...
    documents: (Array.isArray(payload.documents) ? payload.documents : []).filter((document) => document.type === "plan")
  };
}

function scorePack(doc, pack) {
  const corpus = buildPlanCorpus(doc);
...
function assignPlanPacks(documents = []) {
  const packMap = new Map(PLAN_PACK_DEFS.map((pack) => [pack.id, {
    id: pack.id,
    title: pack.title,
  ...
  packMap.set("general", {
    id: "general",
    title: "General / Misc",
    summary: "Plans that do not strongly match a narrower pack.",
    preferredRoles: [],
    keywords: [],
    count: 0,
    documents: []
  ...
  for (const document of documents) {
    let bestId = "general";
    let bestScore = 0;
    for (const pack of PLAN_PACK_DEFS) {
...
function buildPlanPackReport(options = {}) {
  const payload = planDocsFromPayload(options);
  const packs = assignPlanPacks(payload.documents);
  return {
    ok: true,
    profile: payload.profile,
    totalPlans: payload.documents.length,
...
function getPlanPacksForRole(role, config, options = {}) {
  const report = options.report && options.report.packs ? options.report : buildPlanPackReport(options);
  ...
  return {
    ok: true,
    role,
    totalPlanPacks: report.packCount,
    packs: selected
  };
}

function buildPlanPointersForRole(role, config, options = {}) {
  const report = options.report && options.report.packs ? options.report : buildPlanPackReport(options);
  const selected = Array.isArray(options.visiblePacks) && options.visiblePacks.length > 0
    ? options.visiblePacks
    : getPlanPacksForRole(role, config, { limit: options.limit || 2, report }).packs;
  ...
  let guidance = "Use focused plan anchors before widening to raw plan search.";
  if (primaryPack) {
  ...
  return {
    ok: true,
    role,
    totalPlans: Number(report.totalPlans || 0),
...
function renderPlanPackCatalog(report = buildPlanPackReport()) {
  const lines = [
    "# Plan Packs",
    "",
    "> Auto-generated. Do NOT hand-edit.",
    `> Profile: ${report.profile} | Plans: ${report.totalPlans} | Packs: ${report.packCount}`,
    "",
    "## Packs",
    ""
  ];
...
function renderPlanPointerCatalog(report) {
  const pointerReport = report && Array.isArray(report.roles)
    ? report
    : buildPlanPointerReport(report || {});
  const lines = [
    "# Plan Pointers",
    "",
    "> Auto-generated. Do NOT hand-edit.",
    `> Profile: ${pointerReport.profile} | Plans: ${pointerReport.totalPlans} | Packs: ${pointerReport.packCount} | Roles: ${pointerReport.roleCount}`,
    "",
    "## Roles",
    ""
  ];
...
function writePlanPackCatalog(report = buildPlanPackReport(), outputPath = DEFAULT_OUTPUT_PATH) {
  const content = renderPlanPackCatalog(report);
...
function writePlanPointerCatalog(report, outputPath = DEFAULT_POINTER_OUTPUT_PATH) {
  const content = renderPlanPointerCatalog(report);
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, content, "utf8");
  return {
    ok: true,
...
module.exports = {
  ...
  PLAN_PACK_DEFS,
  buildPlanPackReport,
  buildPlanPointerReport,
  buildPlanPointersForRole,
  getPlanPacksForRole,
  renderPlanPackCatalog,
  renderPlanPointerCatalog,
  writePlanPackCatalog,
  writePlanPointerCatalog
};
